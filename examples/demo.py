...
def demo_simple_oracle():
    ...
    learner = ActiveLearner(
        d=3, oracle=oracle, n_models=3, hidden_dims=[32, 32], device="cpu", seed=42
    )

    # Run active learning
    print("\nRunning 20 iterations...")
    results = learner.run(
        n_iterations=20, n_candidates=500, n_initial=5, n_oracle_queries=3, verbose=False
    )
    ...
    for i in range(5):
        ...
        print(
            f"[{x[0]:.2f}, {x[1]:.2f}, {x[2]:.2f}] | {true_f:.3f}     | {pred_f:.3f}     | {correct}"
        )
...
def demo_comparison():
    ...
    learner = ActiveLearner(d=3, oracle=oracle, n_models=3, device="cpu", seed=42)

    results = learner.run(
        n_iterations=15, n_candidates=500, n_initial=5, n_oracle_queries=1, verbose=False
    )
