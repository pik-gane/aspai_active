...
def visualize_results(learner, true_function, results, save_path="results.png"):
    ...
    scatter = ax.scatter(
        coords_2d[:, 0], coords_2d[:, 1], c=colors, cmap="RdYlGn", s=20, alpha=0.6, vmin=0, vmax=1
    )
    ax.plot(*train_coords_2d.T, "k.", markersize=8, alpha=0.7, label="Query points")
    ax.add_patch(Polygon(triangle, fill=False, edgecolor="black", linewidth=2))
    ax.set_xlim(-0.1, 1.1)
...
def main():
    ...
    learner = ActiveLearner(
        d=d, oracle=oracle, n_models=5, hidden_dims=[64, 64], device=device, seed=seed
    )
    ...
    fig = visualize_results(learner, true_function, results, save_path="example_results.png")
    plt.close(fig)
