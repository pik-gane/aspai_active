...
def run_experiment(d, n_iterations, optimize_candidates, seed=None):
    ...
    learner = ActiveLearner(
        d=d, oracle=oracle, n_models=5, hidden_dims=[128, 64], device=device, seed=seed
    )
...
def compare_methods(d=20, n_iterations=30, n_trials=3):
    ...
    for trial in range(n_trials):
        ...
        result_without = run_experiment(
            d=d, n_iterations=n_iterations, optimize_candidates=False, seed=seed
        )
        ...
        result_with = run_experiment(
            d=d, n_iterations=n_iterations, optimize_candidates=True, seed=seed
        )
    ...
    print("=" * 70)
    print("Without gradient optimization:")
    print(
        f"  Mean accuracy: {np.mean(accuracies_without_opt):.2%}"
        f" ± {np.std(accuracies_without_opt):.2%}"
    )
    print()
    print("With gradient optimization:")
    print(
        f"  Mean accuracy: {np.mean(accuracies_with_opt):.2%}"
        f" ± {np.std(accuracies_with_opt):.2%}"
    )
    print()
...
def main():
    ...
    """
    compare_methods(d=20, n_iterations=30, n_trials=3)
